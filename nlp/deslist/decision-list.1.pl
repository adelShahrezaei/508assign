...
sub getFeatures {
    ...
    my @tokens = split(" ",%{$instance}{"context"});
    # print Dumper(\@tokens);
    foreach my $token (@tokens){
       
        $mostFreq{$senseID}{$token}++;
    }
    ...
    if (%{$instance}{"context"} =~ /the/gm ){

        $colFeatures[5]{1}{$senseID}++;

    }else{
        $colFeatures[5]{0}{$senseID}++;
    }
...
# foreach my $name (sort { $mostFreq{"phone"}{$a} <=> $mostFreq{"phone"}{$b} } keys %{$mostFreq{"phone"}}) {
#     printf $name."  ".$mostFreq{"phone"}{$name}."\n";
# }

#telephone # phone #phone 

# foreach my $name (sort { $mostFreq{"product"}{$a} <=> $mostFreq{"product"}{$b} } keys %{$mostFreq{"product"}}) {
#     printf $name."  ".$mostFreq{"product"}{$name}."\n";
# }
